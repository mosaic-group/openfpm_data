...
public:
  ...
  template<typename S> grid(const grid<N,S> & g)
  {
  ...
  inline grid<N,T> & operator=(const grid<N,T> & g)
  {
	  box = g.box;
	  size_tot = g.size_tot;
...
public:
	 ...
	 * \param g Grid on which iterate
	 */
	template<typename T> grid_key_dx_iterator(const grid<dim,T> & g)
	: grid_base(g)
	{
		//! Initialize to 0 the index
...
public:
	 ...
	 *
	 */
	template<typename T> grid_key_dx_iterator_sub(const grid<dim,T> & g, const grid_key_dx<dim> & start, const grid_key_dx<dim> & stop)
	 ...
	 *
	 */
	template<typename T> grid_key_dx_iterator_sub(const grid<dim,T> & g, size_t start[dim], size_t stop[dim])
	: grid_key_dx_iterator<dim>(g),grid_base(g),gk_start(start), gk_stop(stop)
	{
#ifndef DEBUG
		//! If we are on debug check that the stop grid_key id bigger than the start
