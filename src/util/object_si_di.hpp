...
struct object_si_di_e
{
	 ...
	 *
	 */
	object_si_di_e(v_src && src, v_dst & dst)
	:src(src),dst(dst)
	{std::cerr << "Error: " <<__FILE__ << ":" << __LINE__ << " Passing a temporal object\n";};
#endif
    ...
    __device__ __host__ void operator()(T& t)
    {
    	...
    	meta_copy_d<copy_stype,copy_dtype>::meta_copy_d_(src.template get<boost::mpl::at<v_prp,boost::mpl::int_<T::value>>::type::value>(),dst.template get<boost::mpl::at<v_prp,boost::mpl::int_<T::value>>::type::value>());
    }
};
...
struct object_si_di
{
	 ...
	 *
	 */
	inline object_si_di(const v_src & vs, v_dst & vd)
	{
		std::cerr << "Error object_copy: " << __FILE__ << " " << __LINE__ << "\n";
	};
};
 ...
 */
template<typename v_src, typename v_dst, int... prp>
struct object_si_di<v_src,v_dst,OBJ_ENCAP,prp...>
{
	/*! \brief Implementation of the copy with operation
	 *
	 * \param vs source object
	 ...
	 *
	 */
	__device__ __host__ inline object_si_di(const v_src & vs, v_dst && vd)
	{
		object_si_di_e<v_src,v_dst,prp...> obj(vs,vd);
		boost::mpl::for_each_ref< boost::mpl::range_c<int,0,sizeof...(prp)> >(obj);
	}
