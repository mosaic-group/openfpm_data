...
#include "boost_multi_array_ref_openfpm.hpp"
...
namespace boost {
  namespace detail {
    namespace multi_array {
      ...
      struct populate_index_ranges_openfpm {
...
class multi_array_openfpm :
  public multi_array_ref_openfpm<T,NumDims>
{
  typedef multi_array_ref_openfpm<T,NumDims> super_type;
public:
  ...
  template <class ExtentList>
  explicit multi_array_openfpm(ExtentList const& extents,
  ...
  explicit multi_array_openfpm(const detail::multi_array
  ...
  multi_array_openfpm(const multi_array_openfpm& rhs) :
  ...
  multi_array_openfpm(const const_multi_array_ref_openfpm<T,NumDims,OPtr>& rhs,
  ...
  template <typename OPtr>
  multi_array_openfpm(const detail::multi_array::
  ...
  multi_array_openfpm(const multi_array_ref_openfpm<T,NumDims>& rhs)
    : super_type(0,c_storage_order(),rhs.index_bases(),rhs.shape())
  {
    allocate_space();
    // Warning! storage order may change, hence the following copy technique.
  ...
  multi_array_openfpm& operator=(const ConstMultiArray& other) {
    super_type::operator=(other);
    return *this;
  }

  multi_array_openfpm& operator=(const multi_array_openfpm& other) {
    if (&other != this) {
      super_type::operator=(other);
    }
    return *this;
  ...
  multi_array_openfpm& resize(const ExtentList& extents) {
    boost::function_requires<
      detail::multi_array::CollectionConcept<ExtentList> >();
  ...
  multi_array_openfpm& resize(const detail::multi_array
                      ::extent_gen<NumDims>& ranges) {


    // build a multi_array with the specs given
    multi_array_openfpm new_array(ranges,this->storage_order());
    ...
    typename
      multi_array_openfpm::BOOST_NESTED_TEMPLATE array_view_openfpm<NumDims>::type view_old = (*this)[old_idxes];
    typename
      multi_array_openfpm::BOOST_NESTED_TEMPLATE array_view_openfpm<NumDims>::type view_new = new_array[new_idxes];
...
#endif /* OPENFPM_DATA_SRC_UTIL_BOOST_MULTI_ARRAY_OPENFPM_HPP_ */
