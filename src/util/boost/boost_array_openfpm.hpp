 ...
 * Jan 29, 2004
 */
#ifndef BOOST_ARRAY_HPP_OPENFPM
#define BOOST_ARRAY_HPP_OPENFPM
...
namespace boost {
    ...
    class array_openfpm {
      ...
      public:
        ...
        __device__ __host__ const T* data() const { return elems; }
    ...
    bool operator== (const array_openfpm<T,N>& x, const array_openfpm<T,N>& y) {
    ...
    // global swap()
    template<class T, std::size_t N>
    inline void swap (array_openfpm<T,N>& x, array_openfpm<T,N>& y) {
        x.swap(y);
    }
...
// Specific for boost::array: simply returns its elems data member.
    template <typename T, std::size_t N>
    T(&get_c_array(boost::array_openfpm<T,N>& arg))[N]
    {
        return arg.elems;
    }
    ...
    // Const version.
    template <typename T, std::size_t N>
    const T(&get_c_array(const boost::array_openfpm<T,N>& arg))[N]
    {
        return arg.elems;
    }
#endif
    ...
    template<class T, std::size_t N>
    std::size_t hash_value(const array_openfpm<T,N>& arr)
    {
        return boost::hash_range(arr.begin(), arr.end());
    }

   template <size_t Idx, typename T, size_t N>
   T &get(boost::array_openfpm<T,N> &arr) BOOST_NOEXCEPT {
       BOOST_STATIC_ASSERT_MSG ( Idx < N, "boost::get<>(boost::array &) index out of range" );
       return arr[Idx];
       }

   template <size_t Idx, typename T, size_t N>
   const T &get(const boost::array_openfpm<T,N> &arr) BOOST_NOEXCEPT {
