...
public:
	inline __device__ NN_gpu_it_symm(
		size_t p,
		const grid_key_dx<dim,ids_type> & cellPosition,
		const openfpm::vector_gpu_ker<aggregate<unsigned int>,memory_traits_inte> & numPartInCellPrefixSum,
		const openfpm::vector_gpu_ker<aggregate<unsigned int>,memory_traits_inte> & unsortedToSortedIndex,
	...
	neighborCellIndexAct(neighborCellOffset.template get<0>(0))
	{
		unsigned neighborPartIndexStartOld = numPartInCellPrefixSum.template get<0>(cellPositionIndex+neighborCellIndexAct);
		neighborPartIndexStart = unsortedToSortedIndex.template get<0>(p);
		neighborPartIndexStop = numPartInCellPrefixSum.template get<0>(cellPositionIndex+neighborCellIndexAct+1);

		// printf("neighborPartIndexStartOld %u  neighborPartIndexStart %u neighborPartIndexStop %u\n", neighborPartIndexStartOld, neighborPartIndexStart, neighborPartIndexStop );
	...
	inline __device__ NN_gpu_it_symm<dim,ids_type> & operator++()
	{
		++neighborPartIndexStart; SelectValid();
...
public:
	...
	inline __device__ NN_gpu_it_symm<dim,ids_type> getNNIteratorRadiusSymm(
		size_t p,
		const grid_key_dx<dim,ids_type> & cellPosition)
	{
		NN_gpu_it_symm<dim,ids_type> ngi(p, cellPosition,numPartInCellPrefixSum,unsortedToSortedIndex,sortedToUnsortedIndex,rcutNeighborCellOffset,this->get_div_c());

		return ngi;
	}

	inline __device__ NN_gpu_it_symm<dim,ids_type> getNNIteratorBoxSymm(
		size_t p,
		const grid_key_dx<dim,ids_type> & cellPosition)
	{
		NN_gpu_it_symm<dim,ids_type> ngi(p, cellPosition,numPartInCellPrefixSum,unsortedToSortedIndex,sortedToUnsortedIndex,boxNeighborCellOffset,this->get_div_c());
