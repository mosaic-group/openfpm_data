...
/*	CellNNIteratorSym<dim,CellListType,RUNTIME,NO_CHECK> getNNIteratorCSR(const openfpm::vector<Point<dim,typename CellListType::stype>> & v) const
	...
	{
		if (dom_or_anom == 0)
			return typename CellListType::SymNNIterator(dom_cell.get(cid),*start,NNc_sym,openfpm::math::pow(3,dim)/2+1,cli,pos,v);
		else
			return typename CellListType::SymNNIterator(anom_dom_cell.get(cid).subsub,*start,&anom_dom_cell.get(cid).NN_subsub.get(0),anom_dom_cell.get(cid).NN_subsub.size(),cli,pos,v);
	}
};
