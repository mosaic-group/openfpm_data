...
class vector_ofp
{
    ...
    //! 1-D static grid
    vector<aggregate<T>,HeapMemory,typename memory_traits_lin<aggregate<T>>::type,memory_traits_lin,grow_policy_identity> base;
...
public:
     ...
     *
     */
    inline void add(const openfpm::vector_ofp<T> & eles)
    {
        ...
        {
            base.template get<0>(i) = eles.base.template get<0>(i-start);
        }
    }
    ...
    */
    ...
    //! Constructor, vector of size sz
    vector_ofp(size_t sz) noexcept
            :base(sz),err_code(0)
    {
    }

    //! Constructor from another vector
    vector_ofp(const vector_ofp<T> & v) noexcept
            :err_code(0)
    {
        base = v.base;
    }
     ...
     *
     */
    vector_ofp(const std::initializer_list<T> & v)
            :base(v)
    {
    }

    //! Constructor from another vector
    vector_ofp(vector_ofp<T> && v) noexcept
            :err_code(0)
    {
        base.swap(v.base);
    }
 ...
 */
    ...
    {
        base.template packRequest<0>(req);
    }
