...
void test3(layout_gpu< grid<Point<float>>, memory_gpu<memory_gpu_type<Point<float>>::type> > & c3)
{
  std::cout << "3D Array with grid_key_3 storing property: " << "\n";
  ...
  for (int i = 0 ; i < GS_SIZE ; i++)
  {
    for (int j = 0 ; j < GS_SIZE ; j++)
    {
      ...
      {
    	  grid_key_3<P::x> kx(i,j,k);
    	  grid_key_3<P::y> ky(i,j,k);
    	  grid_key_3<P::z> kz(i,j,k);
    	  grid_key_3<P::s> ks(i,j,k);
    	  grid_key_3<P::v> kv(i,j,k);
    	  grid_key_3<P::t> kt(i,j,k);
...
void test3(layout_cpu< grid<Point<float>>, memory_cpu<memory_cpu_type<Point<float>>::type> > & c3)
{
  std::cout << "3D Array with grid_key_3 storing property: " << "\n";
